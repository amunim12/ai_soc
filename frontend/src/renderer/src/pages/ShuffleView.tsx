...
export default function ShuffleView({ onBack }: ShuffleViewProps) {
  ...
  const fetchTelemetry = useCallback(async () => {
    setLoading(true)
    try {
      const [execs, tele] = await Promise.all([api.soarExecutions(150), api.soarTelemetry(24)])
      setExecutions(execs)
      setTelemetry(tele)
    } catch {
      /* backend offline */
  ...
  const totalFinished = executions.filter((e) =>
    ['FINISHED', 'COMPLETED'].includes(e.status)
  ).length
  ...
  const successRate = executions.length ? Math.round((totalFinished / executions.length) * 100) : 0
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 20 }} className="animate-in">
          ...
          {workflows.length === 0 ? (
            <div className="empty-state">
              ...
              <span>
                No workflows found — run{' '}
                <code style={{ color: '#94a3b8' }}>register_shuffle_workflows.py</code>
              </span>
            </div>
          ) : (
            <div
            ...
            >
              {workflows.map((wf) => (
                ...
                >
                  ...
                  >
                    <div
                      style={{ fontSize: 14, fontWeight: 700, color: '#e2e8f0', lineHeight: 1.3 }}
                    >
                      {wf.name}
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: 16, fontSize: 11 }}>
                    {[
                      ...
                      >
                        ...
                        <span style={{ fontWeight: 700, fontSize: 16, color: s.color }}>
                          {s.value}
                        </span>
                      </div>
                    ))}
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: 20 }} className="animate-in">
            ...
            >
              {[
                ...
                { label: 'In Progress', value: totalExecuting, color: '#38bdf8' },
                {
                  label: 'Success Rate',
                  value: `${successRate}%`,
                  color: successRate >= 80 ? '#34d399' : '#fbbf24'
                }
              ].map((s) => (
            ...
            <div className="glass" style={{ overflow: 'hidden' }}>
              <div
                style={{
                  display: 'grid',
                  gridTemplateColumns: '1fr 110px 90px 80px 70px',
                  gap: '0 10px',
                  padding: '9px 14px',
                  background: 'rgba(255,255,255,0.03)',
              ...
              {executions.length === 0 ? (
                ...
                executions.map((ex, i) => (
                  <div
                    ...
                    onClick={() =>
                      setSelected(selected?.execution_id === ex.execution_id ? null : ex)
                    }
                    style={{
                      display: 'grid',
                      gridTemplateColumns: '1fr 110px 90px 80px 70px',
                      ...
                      padding: '10px 14px',
                      borderBottom:
                        i < executions.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none',
                      fontSize: 12,
                      alignItems: 'center',
                      cursor: 'pointer',
                      background:
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      padding: '10px 14px',
                      borderBottom:
                        i < audit.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none',
                      fontSize: 11,
                      alignItems: 'center'
                    }}
                  >
