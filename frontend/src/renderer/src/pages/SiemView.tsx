...
import { api, SiemSummary, SiemAlert, SeverityPoint, MitreStat, AgentRow } from '../lib/api'
...
export default function SiemView({ onBack }: SiemViewProps) {
  ...
  const setLoading = (t: Tab, v: boolean) => setLoadingMap((prev) => ({ ...prev, [t]: v }))
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div
        className="topbar"
        style={{ paddingLeft: 20, gap: 10, flexShrink: 0, flexWrap: 'wrap', minHeight: 52 }}
      >
        {onBack && (
          <button
      ...
      {tab === 'overview' && (
        <div style={{ flex: 1, overflow: 'auto', padding: 20 }} className="animate-in">
          ...
          >
            ...
            <StatCard
              label="Executed"
              value={summary?.executed_playbooks ?? 0}
              color="#34d399"
              icon="▶"
            ...
            <StatCard
              label="Dropped"
              value={summary?.dropped_alerts ?? 0}
              color="#64748b"
              icon="🚫"
              sub="Noise / duplicates"
            />
            <StatCard
              label="Escalated"
            ...
            <StatCard
              label="Agents Active"
              value={`${summary?.agents_active ?? 0}/${summary?.agents_total ?? 0}`}
              color="#38bdf8"
              icon="⬡"
              sub="Wazuh endpoints"
            />
                ...
                <>
                  <div
                    style={{
                      fontSize: 40,
                      fontWeight: 900,
                      color: summary.avg_confidence >= 0.85 ? '#34d399' : '#fbbf24',
                      lineHeight: 1
                    }}
                  >
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '0 20px 20px' }}>
            {filteredAlerts.length === 0 ? (
              <div className="empty-state">
                <div className="empty-state-icon">✓</div>
                <span>
                  {alerts.length === 0 ? 'No alerts in pipeline yet' : 'No alerts match filter'}
                </span>
              </div>
            ) : (
                ...
                {filteredAlerts.map((a, i) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: '10px 14px',
                      borderBottom:
                        i < filteredAlerts.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none',
                      fontSize: 12,
                      alignItems: 'center',
                  ...
                  >
                    ...
                    <div
                      style={{ fontSize: 11, color: a.confidence >= 0.85 ? '#34d399' : '#fbbf24' }}
                    >
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 12, color: '#64748b' }}>
                {summary?.wazuh_online
                  ? 'Live from Wazuh Manager REST API'
                  : 'Derived from pipeline alert data'}{' '}
                · refreshes every 12 s
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              {[
                ...
                {
                  label: 'Disconnected',
                  count: agents.filter((a) => a.status === 'disconnected').length,
                  color: '#f87171'
                },
                {
                  label: 'Never Seen',
                  count: agents.filter((a) => a.status === 'never_connected').length,
                  color: '#64748b'
                }
              ].map((s) => (
                <span
      ...
      {tab === 'mitre' && (
        <div style={{ flex: 1, overflow: 'auto', padding: 20 }} className="animate-in">
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 12, color: '#64748b' }}>
              Top attack techniques detected across all pipeline alerts
            </div>
          </div>
                ...
                <ResponsiveContainer width="100%" height={Math.max(240, mitre.length * 26)}>
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      formatter={(v, _n, p) => [
                        v,
                        p.payload.technique_name || p.payload.technique_id
                      ]}
                    />
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <span style={{ fontSize: 12, color: '#94a3b8' }}>
                            {item.technique_name !== item.technique_id ? item.technique_name : '—'}
                          </span>
                        </div>
                        <span style={{ fontSize: 13, fontWeight: 800, color: riskColor }}>
                          {item.count}
                        </span>
                      </div>
                      {/* Frequency bar */}
                      <div
                        style={{ height: 3, background: 'rgba(255,255,255,0.05)', borderRadius: 2 }}
                      >
                        <div
                          style={{
                            height: '100%',
