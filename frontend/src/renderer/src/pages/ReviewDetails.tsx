...
export default function ReviewDetails({ review, onBack, onDecisionMade }: ReviewDetailsProps) {
  ...
  try { alert = JSON.parse(review.alert_json) } catch { /* empty */ }
  try { playbook = JSON.parse(review.playbook_json) } catch { /* empty */ }

  const phases = Object.entries(playbook.phases ?? {})
  const uniqueMitre = [...new Set(
    phases.flatMap(([, p]) => p.steps.filter(s => s.mitre_technique).map(s => s.mitre_technique as string))
  )]
  ...
  return (
    <div className="page-content animate-slide">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, color: '#e2e8f0' }}>Threat Investigation</div>
          <div style={{ fontSize: 11, fontFamily: 'monospace', color: '#64748b' }}>{review.review_id}</div>
        </div>
      </div>
          ...
          <div className="glass" style={{ padding: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 12px', fontSize: 12 }}>
              {[
                ...
                <div key={k}>
                  <div style={{ color: '#64748b', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{k}</div>
            ...
            {uniqueMitre.length > 0 && (
              <div style={{ marginTop: 12, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {uniqueMitre.map(t => <MitreBadge key={t} technique={t} />)}
              </div>
            )}
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="glass" style={{ padding: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              <div className="section-title" style={{ marginBottom: 0 }}>Generated Playbook</div>
                ...
                {(['visual', 'yaml'] as PlaybookTab[]).map(tab => (
                  <button
                    ...
                    style={{
                      ...
                      borderColor: playbookTab === tab ? 'rgba(139,92,246,0.4)' : 'rgba(255,255,255,0.06)',
            ...
            {playbook.summary && (
              <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 8 }}>{playbook.summary}</div>
            )}
            <div style={{ display: 'flex', gap: 8 }}>
              <span style={{ fontSize: 11, color: '#64748b' }}>
                Confidence:{' '}
                <span style={{ fontWeight: 700, color: (playbook.confidence_score ?? 0) >= 0.85 ? '#34d399' : '#fbbf24' }}>
                  {Math.round((playbook.confidence_score ?? 0) * 100)}%
                </span>
              </span>
              <span style={{ color: '#334155' }}>·</span>
              <span style={{ fontSize: 11, color: '#64748b' }}>
                Est: <span style={{ color: '#94a3b8', fontWeight: 600 }}>{playbook.estimated_duration_minutes ?? '?'} min</span>
              </span>
            </div>
          </div>
          ...
          <div className="glass" style={{ overflow: 'hidden', flex: 1 }}>
            {playbookTab === 'yaml' ? (
              <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                ...
                >
                  <span>✎ Live YAML Editor — submit with action "Edit" to apply changes</span>
                  <button
              ...
              phases.map(([phaseName, phase]) => (
                <div key={phaseName}>
                  <div className="phase-header" style={{ color: PHASE_COLORS[phaseName] ?? '#94a3b8' }}>
                    <span>{PHASE_ICONS[phaseName] ?? '◈'}</span>
                    <span>{phase.phase_name ?? phaseName}</span>
                    <span style={{ marginLeft: 'auto', fontSize: 10, color: '#475569', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>
                  ...
                  {phase.steps.map(step => (
                    <div key={step.step_id} className="playbook-step">
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12, color: '#e2e8f0', marginBottom: 3 }}>{step.description}</div>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
                          ...
                          {step.requires_confirmation && (
                            <span style={{ fontSize: 10, padding: '1px 5px', background: 'rgba(251,191,36,0.12)', color: '#fbbf24', borderRadius: 3, border: '1px solid rgba(251,191,36,0.2)' }}>
                              ⚠ confirm
                            </span>
                          )}
                        </div>
          ...
          <div className="glass" style={{ padding: 16 }}>
            ...
            {result ? (
              <div style={{ padding: '12px 14px', background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.25)', borderRadius: 6, color: '#34d399', fontSize: 12 }}>
                ✓ {result}
              </div>
            ) : (
              <>
                ...
                {error && (
                  <div style={{ padding: '8px 12px', background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 6, color: '#f87171', fontSize: 11, marginBottom: 10 }}>
                    {error}
                  </div>
                )}
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <button className="btn btn-approve" onClick={() => submit('APPROVE')} disabled={submitting !== null} style={{ flex: 1, justifyContent: 'center' }}>
                    {submitting === 'APPROVE' ? <span className="spinner" /> : '✓'} Approve
                  </button>
                  {playbookTab === 'yaml' && (
                    <button className="btn btn-primary" onClick={() => submit('EDIT')} disabled={submitting !== null} style={{ flex: 1, justifyContent: 'center' }}>
                      {submitting === 'EDIT' ? <span className="spinner" /> : '✎'} Submit Edit
                    </button>
                  )}
                  <button className="btn btn-escalate" onClick={() => submit('ESCALATE')} disabled={submitting !== null} style={{ flex: 1, justifyContent: 'center' }}>
                    {submitting === 'ESCALATE' ? <span className="spinner" /> : '⬆'} Escalate
                  </button>
                  <button className="btn btn-reject" onClick={() => submit('REJECT')} disabled={submitting !== null} style={{ flex: 1, justifyContent: 'center' }}>
